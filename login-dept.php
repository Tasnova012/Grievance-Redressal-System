<?php
    ...
    if( isset($_POST['login']) ) {    
        ...
        $activity = "New Login";
        $details = "New Login. Email : ".$email;       

        if (!$error) {     

            $stmt = $conn->prepare("SELECT * FROM department WHERE email=? AND status='Active'");
            $stmt->execute([$email]);
            ...
            if ($stmt->rowCount() === 1) {
                ...
                if ($email === $dbemail || $email === $dbstudent_id) {

                    if (password_verify($password, $dbpassword)) {
                        $_SESSION['logged_in'] = $dbemail;
                        $_SESSION['role'] = "Dept";
                        header("Location: home.php");
                            ...
                            <div class="card-body">
                                <div class="px-2 py-3">
                                    ...
                                    <div class="text-center">
                                        ...
                                        <h5 class="text-primary mb-2 mt-4">Department Login Panel</h5>
                                    </div>
