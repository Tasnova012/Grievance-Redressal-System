                                    ...
                                    <div class="card">
                                        <div class="card-body  pt-0">
                                            <ul class="nav nav-tabs nav-tabs-custom mb-4">
                                                <li class="nav-item">
                                                    <a class="nav-link fw-bold p-3 active" href="#">All Proctor</a>
                                                </li>
                                            </ul>                                              
                                            ...
                                            <div class="table-responsive">
                                                ...
                                                <?php											
                                                    ...
                                                    $members = $conn->query("SELECT * FROM proctors ORDER BY id DESC");
                                                    while ($row = $members->fetch(PDO::FETCH_OBJ)){	                                                        
                                                        $id = $row->id;

                                                        if($row->grc){
                                                            $grc_email = $row->grc;
                                                            $grc = $conn->prepare("SELECT * FROM grc WHERE email = :email");
                                                            $grc->execute(['email' => $grc_email]); 
                                                            $rows = $grc->fetch(PDO::FETCH_OBJ);
                                                        ...
                                                        ."<td>". $full_name ."</td>"
                                                        ."<td>
                                                            <a href='edit-proctor.php?id=". $id ."' class='btn btn-sm btn-primary'><i class='fas fa-edit'></i></a>											
...
<?php include 'footer.php'; ?>
    ...
    <script>
        ...
        function delete_staff($id){
            $.ajax({
                url:"process/action.php?action=delete_proctor",
                method:'POST',
                data:{id:$id},
